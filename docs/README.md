...
>The Naver Authenticator is supported from WSO2 Identity Server 5.10.0 onwards.
...
3. Sign up to Naver using the following URL : `https://nid.naver.com/user2/V2Join.nhn?m=agree&lang=en_US&cpno=`
...
3. Enter a suitable name as the **Identity Provider Name** (e.g., Naver).
    >As our resident Identity Provider is
 WSO2 IS, the Alias will appear as follows - `https://(host-name):(port)/oauth2/token`
   
4. Navigate to the **NAVER Authenticator Configurations** under **Federated Authenticators**.
...
5. Enter the IdP related details as follows.
    ...
    <tr class="odd">
    <td>Enable</td>
    <td>Selecting this option enables Naver to be used as an authenticator for users provisioned to the Identity
     Server.</td>
    <td>Selected</td>
    </tr>
    <tr class="even">
    <td>Default</td>
    <td>Selecting the Default checkbox signifies that Naver is the main/default form of authentication. 
...
5. Add the service provider details as follows.
    ...
    g. Configure the Local and Outbound Authentication for Naver.
